...
- **Docker socket detection order matters**: On macOS with both Docker Desktop and Colima installed, the Colima socket (`~/.colima/default/docker.sock`) exists on disk but Docker Desktop can't mount it inside its VM. Solution: check `/var/run/docker.sock` first (universal symlink all runtimes create), then fall back to runtime-specific paths.
